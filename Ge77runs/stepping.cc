...
void MySteppingAction::UserSteppingAction(const G4Step *step){
    ...
    if(volume == fScoringVolume)
    {
        ...
        if(particle == requiredParticle)
            G4cout << "Neutron left Volume" << G4endl;
        return;
    }
    ...
    if(particle == requiredParticle && step->GetTrack()->GetTrackStatus() == fStopAndKill && process->GetProcessName() == "nCapture" && track->GetParentID() == 0){
